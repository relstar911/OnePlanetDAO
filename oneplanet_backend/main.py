...
app = FastAPI(
    ...
    description=(
        "API for One Planet governance, identity, tokenomics, and reporting, strictly aligned "
        "with project documentation in /docs."
    ),
    version="0.1.0",
)
...
async def ratelimit_handler(request: Request, exc: RateLimitExceeded):
    return JSONResponse(
        status_code=429,
        content={
            "detail": ("Rate limit exceeded. Bitte warte einen Moment und versuche es erneut.")
        },
    )
...
def root():
    return {
        "msg": "Welcome to the One Planet API. See /docs for OpenAPI and project documentation."
    }
