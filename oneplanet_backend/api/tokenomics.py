...
def create_alert(alert: AlertResponse, request: Request, session: Session = Depends(get_session), user=Depends(require_auth)):
    ...
    """
    if alert.epoch is None or alert.msi is None or alert.vei is None or not alert.collusion or not alert.status or not alert.alert:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Alle Felder (epoch, msi, vei, collusion, status, alert) müssen gesetzt sein.")
    if not isinstance(alert.epoch, int) or alert.epoch < 0:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="epoch muss >= 0 sein.")
    if not isinstance(alert.msi, float) or not (0.0 <= alert.msi <= 1.0):
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="msi muss zwischen 0 und 1 liegen.")
    if not isinstance(alert.vei, float) or not (0.0 <= alert.vei <= 1.0):
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="vei muss zwischen 0 und 1 liegen.")
    for field in [alert.collusion, alert.status, alert.alert]:
        if not isinstance(field, str) or not field.strip():
            raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="collusion, status und alert müssen nicht-leere Strings sein.")
